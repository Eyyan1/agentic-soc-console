...
try:
    ...
    from PLUGINS.Embeddings.config_runtime import EMBEDDINGS_TYPE, EMBEDDINGS_BASE_URL, EMBEDDINGS_MODEL, EMBEDDINGS_API_KEY, EMBEDDINGS_SIZE, EMBEDDINGS_PROXY
...
LOCAL_FILES_ONLY = os.getenv("ASF_LOCAL_FILES_ONLY", "0") == "1"
DISABLE_EMBEDDINGS = os.getenv("ASF_DISABLE_EMBEDDINGS", "0") == "1"
...
class NoopEmbeddingsAPI(object):
    def add_document(self, collection_name: str, ids: str, page_content: str, metadata: dict) -> str:
        logger.warning("Embeddings disabled; skipping add_document.")
        return ids

    def delete_document(self, collection_name: str, ids: str) -> bool:
        logger.warning("Embeddings disabled; skipping delete_document.")
        return False

    def search_documents(self, collection_name: str, query: str, k: int):
        logger.warning("Embeddings disabled; returning no search results.")
        return []

    def search_documents_with_rerank(self, collection_name: str, query: str, k: int = 20, top_n: int = 5):
        logger.warning("Embeddings disabled; returning no reranked results.")
        return []


if DISABLE_EMBEDDINGS:
    logger.warning("Embeddings subsystem disabled by ASF_DISABLE_EMBEDDINGS.")
    embedding_api_singleton_qdrant = NoopEmbeddingsAPI()
else:
    try:
        embedding_api_singleton_qdrant = EmbeddingsAPI()
    except Exception as exc:
        logger.warning(f"Embeddings subsystem disabled for local development: {exc}")
        embedding_api_singleton_qdrant = NoopEmbeddingsAPI()
